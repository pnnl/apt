...
class OPTICSAPT(Clustering):
    ...
    def hierarchy_clustering(self, 
        ...
        if min_node_size == None:
            min_node_size = self._paras['OPTICS minpts']
    ...
    def create_clusters(self, est_bg=None, min_cluster_size=None):
        """
        ...
        if min_cluster_size == None:
            min_cluster_size = self._paras['OPTICS minpts']


        cluster_id = 0 # Note default cluster id is -1, which correspond to noise
        hierarchy_leaves = self.obtain_leaf_nodes()
